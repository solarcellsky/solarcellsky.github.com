var tools = {
    ...
    clipfn: function (path, id) {
        var _clip = "clip";
        if (id) _clip = id;
        var clip = new ZeroClipboard(tools.getid(_clip), {
    ...
    transcoding: {
        ...
        lowtoupp: function (path) {
            ...
            forms.clear.onclick = function () { tools.clear([tools.getid('trans'), tools.getid('num')]); }
            ...
            forms.clear1.onclick = function () { tools.clear([tools.getid('content')]); }
        ...
        utf_8: function () {
            ...
            $("#clear").click(function () {
                tools.clear([tools.getid("content"), tools.getid("result")]);
                $(".CentHid").show();
            });
        }
    },
    ...
    encryptDecode: {
        ...
        scriptEncode: function () {
            $("#jsencode").click(function () {
                var v = tools.getid('ipt').value;
                if (!v) return;
                var es = escape(v);
                tools.getid('result').value = "document.write(unescape('{0}'));".format(es);
                if (jQuery("#result").val()) $("#result").siblings().hide();
            });
            $("#jsdecode").click(function () {
                var v = tools.getid('result').value;
                var regex = /unescape\('([a-z%0-9].*)'\)/i;
                if (v.match(regex)) {
                    tools.getid('ipt').value = unescape(RegExp.$1);
                }
                if (jQuery("#ipt").val()) $("#ipt").siblings().hide();
            });
            $("#clear").click(function () {
                tools.clear([tools.getid("ipt"), tools.getid("result")]);
                $(".CentHid").show();
            });
        },
        ...
        nativeAscii: function () {
            $("#nativeConvertAscii").click(function () {
                var nativecode = tools.getid("nativecode").value.split("");
                var ascii = "";
                ...
                tools.getid("asciicode").value = ascii;
                if (jQuery("#asciicode").val()) $("#asciicode").siblings().hide();
            });

            $("#asciiConvertNative").click(function () {
                var asciicode = tools.getid("asciicode").value.split("\\u");
                ...
                tools.getid("nativecode").value = nativeValue;
                if (jQuery("#nativecode").val()) $("#nativecode").siblings().hide();
            });
            $("#clear").click(function () {
                tools.clear([tools.getid("nativecode"), tools.getid("asciicode")]);
                $(".CentHid").show();
            });
        },
        unixtime: {
        ...
        hash: {
            ...
            init: function () {
                ...
                $("#clear").click(function () { tools.clear([tools.getid("result"), tools.getid("content")]) });
            }
        }
    },
    htmlcssjs: {
        htmljs: function () {
            var oresul = tools.getid("oresult");
            var osource = tools.getid("osource");
            oresul.onfocus = oresul.onkeyup = function () {
                tools.getid('re').value = tools.getid('oresult').value.replace(/document.writeln\("/g, "").replace(/"\);/g, "").replace(/\\\"/g, "\"").replace(/\\\'/g, "\'").replace(/\\\//g, "\/").replace(/\\\\/g, "\\");
                if (jQuery("#re").val()) $("#re").siblings().hide();
            }
            osource.onfocus = osource.onkeyup = function () {
                tools.getid('oresult2').value = "document.writeln(\"" + tools.getid('osource').value.replace(/\\/g, "\\\\").replace(/\\/g, "\\/").replace(/\'/g, "\\\'").replace(/\"/g, "\\\"").split('\n').join("\");\ndocument.writeln(\"") + "\");";
        ...
        htmlubb: {
            ...
            htmltoubb: function () {
                str = hcj.htmlubb.pattern(tools.getid("Hsource").value);
                tools.getid("Uresult").value = str;
                if (jQuery("#Uresult").val()) $("#Uresult").siblings().hide();
            },
            ubbtohtml: function () {
                str = hcj.htmlubb.up(tools.getid("Usource").value);
                tools.getid("Hresult").value = str;
                if (jQuery("#Hresult").val()) $("#Hresult").siblings().hide();
            },
            init: function () {
                var Hsource = tools.getid("Hsource");
                var Usource = tools.getid("Usource");
                var _this = this;
        ...
        htmlCodeCov: {
            ...
            htmlCov: function () {
                ...
                var type = tools.getid("html").value;
                switch (type) {
                    ...
                    default: tools.getid("result").value = '转换错误'; break;
                }
                if (jQuery("#result").val()) $("#result").siblings().hide();
            },
            init: function (path) {
                ...
                $("#clear").click(function () { tools.clear([tools.getid("content"), tools.getid("result")]) });
            }
        ...
        jsCodeConfusion: function (path) {
            $("#confused").click(function () {
                ...
                var a = new Date();
                tools.getid("ConfusionAfterCode").value = xx.confusion();
                if (jQuery("#ConfusionAfterCode").val()) $("#ConfusionAfterCode").siblings().hide();
            });
            tools.clipfn(path, "clip");
            $("#clear").click(function () { tools.clear([tools.getid("JScode"), tools.getid("ConfusionAfterCode")]) });
        },
        jstool: {
            ...
            init: function (path, key) {
                ...
                tools.clipfn(path, "clip");
                $("#clear").click(function () { tools.clear([tools.getid("txtInitCode"), tools.getid("txtResultCode")]) });
            }
        },
