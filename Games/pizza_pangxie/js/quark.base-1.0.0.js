...
Quark.createDOMDrawable = function(disObj, imageObj)
{
	...
	if(tag == "canvas")
	{
		...
		if(img)
		{
			...
			var rect = imageObj.rect || [0, 0, w, h,0,0,w,h];
			//console.log("111"+img)	;		
...
Quark.hitTestPoint = function(obj, x, y, usePolyCollision)
{
	...
	//var hit = x >= (b.x-50) && x <= (b.x + b.width+50) && y >= (b.y-50) && y <= b.y + b.height+50;
...
CanvasContext.prototype.draw = function(target)
{
	...
	if(target.mask != null)
	{
		...
		arguments[0] = canvas;
		//console.log("333"+arguments[0])	
		this.context.drawImage.apply(this.context, arguments);
	}else if(target._cache != null)
	{
		//draw cache if exist
		//console.log("444"+target._cache)	
	...
	}else
	{
		...
		if(img != null)
		{
			...
			arguments[0] = img;
			//image,0, 0, 320, 480, 0, 0, 320, 480
			//console.log("555"+arguments[0].src)
			//开始按钮
			/*if(arguments[0].src=='http://127.0.0.1/goldCoin/demo/Image/ui_bar_main.png'){
				arguments[5]=0;
				arguments[6]=0;
				arguments[7]=arguments[3]*2;
				arguments[8]=arguments[4]*2;
			}*/
			//背景
			if(arguments[0].src=='http://127.0.0.1/goldCoin/demo/Image/bg_3_2.png'){
				arguments[5]=0;
				arguments[6]=0;
				arguments[7]=windowW;
				arguments[8]=windowH;
			...
			//UI_bar.png
			/*if(arguments[0].src=='http://127.0.0.1/goldCoin/demo/Image/UI_bar.png'){
				arguments[5]=0;
				arguments[6]=0.65*windowH;
				arguments[7]=arguments[3];
				arguments[8]=arguments[4];
			}*/
			//生命值life.png
			/*if(arguments[0].src=='http://127.0.0.1/goldCoin/demo/Image/life.png'){
				arguments[5]=0.5*windowW;
				arguments[6]=0.8*windowH;
				arguments[7]=arguments[3]*3;
				arguments[8]=arguments[4]*3;
			}
			*/
			//boy basket.png
			this.context.drawImage.apply(this.context, arguments);
			//this.context.drawImage(img,arguments[1],arguments[2])	;		
		}
	}
};
...
CanvasContext.prototype.transform = function(target)
{
	...
	}else
	{
		...
		/*if(target.rotation%360 != 0){
			//ctx.rotate(target.rotation%360*Quark.DEG_TO_RAD);
			 //this.rotationCache = this.rotation;
           target._sr = Math.sin(target.rotation);
			target._cr = Math.cos(target.rotation);

			// get the matrix values of the displayobject based on its transform properties..
			a  =  target._cr * target.scaleX;
			b  =  target._sr * target.scaleX;
			c  = -target._sr * target.scaleY;
			d  =  target._cr * target.scaleY;
			tx =  target.x;
			ty =  target.y;
		} */
...
DisplayObject.prototype._render = function(context)
{
	...
	if(!this.rotation){
		...
		ctx.context.drawImage(this.image,this.x,this.y,this.width,this.height);
		ctx.context.save();
		ctx.context.translate(this.x+this.width*0.5, this.y+this.height*0.5);
		ctx.context.rotate(this.rotation);//旋转47度
		ctx.context.translate(-(this.x+this.width*0.5), -(this.y+this.height*0.5));
		ctx.context.drawImage(this.image,this.x,this.y,this.width,this.height);
		//console.log(this)
		//this.render(ctx);
...
{	
	...
	//this.setTargtRect(props.rect || [0, 0, this.image.width, this.image.height]);	
	this.setTargetRect(props.targetRect || [0, 0, this.image.width, this.image.height])
...
Bitmap.prototype.setTargetRect = function(rect)
{
	this.targetRectX = rect[0];
	this.targetRectY = rect[1];
	this.targetRectWidth =  rect[2];
	this.targetRectHeight =  rect[3];
};
/**
...
Bitmap.prototype.render = function(context)
{
	...
	context.draw(this, this.rectX, this.rectY, this.rectWidth, this.rectHeight, this.targetRectX||0, this.targetRectY||0, this.targetRectWidth||this.width, this.targetRectHeight||this.height);
};
 ...
 * @name MovieClip
 * @augments Bitmap
 * @class MovieClip影片剪辑类，表示一组动画片段。
 MovieClip是由Image对象的若干矩形区域组成的集合序列，
 并按照一定规则顺序播放。
...
MovieClip.prototype.setFrame = function(frame, index)
{
	...
	if(index == 0 && this.currentFrame == 0) {
		this.setRect(frame.rect);
		this.setTargetRect(frame.targetRect||frame.rect);
	}
