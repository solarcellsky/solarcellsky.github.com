...
~ function(b) {
    b.load = function() {
        ...
        b.queue.addEventListener("progress", function(c) {
            btGame.gameLoading(c.loaded);
            // updateShare(0);
        });
        b.queue.addEventListener("complete", function(c) {
            Main.log("finish", c)
        });
        b.queue.addEventListener("error", function(c) {
...
Main.endGame = function(b) {
    ...
    q.x = 100;
    q.y = 390;
    var n = "我家母鸡下了" + Main.nowScore + "个蛋\n给你当圣诞礼物也是极好的！\n扑扑星在此\n祝大家圣诞节快乐！";
    d.addChild(q);
    // updateShare(Main.nowScore);
    // Play68.setRankingScoreDesc(Main.nowScore);
    q.text = n
};
...
Main.initGame = function() {
    ...
    Main.initEvent(c);
    createjs.Ticker.addEventListener("tick", function(l) {
...
Main.initEvent = function(e) {
    ...
    e.on("pressmove", function(f) {
        Main.isPlaying && Main.prepareFight()
    });
    e.on("pressup", function(f) {
...
Main.wingMovie = function() {
    rightWing = Main.rightWing, leftWing = Main.leftWing, rightWing.rotation += Main.RightWingRFlag;
...
Main.prepareFight = function() {
    if (Main.checkTouch == false) {
        Main.bear.visible = !1, Main.bearFight.visible = !0;
        var c = Main.rightWing;
        var b = Main.leftWing;
        c.y = b.y = 820, b.x = 140, Main.checkTouch = true
    }
};
