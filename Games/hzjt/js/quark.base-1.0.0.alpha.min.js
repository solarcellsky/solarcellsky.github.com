(function(c) {
	...
	b.merge = function(a, b, c) {
		for (var e in b)
			if (!c || a.hasOwnProperty(e)) a[e] = b[e];
		return a
	};
	...
	b.createDOM = function(a, b) {
		...
		for (e in b) {
			var h = b[e];
			if (e == "style")
				for (var i in h) c.style[i] = h[i];
			else c[e] = h
		}
		return c
	};
	b.use = function(a) {
		for (var a = a.split("."), b = c, d = 0; d < a.length; d++) var e = a[d],
			b = b[e] || (b[e] = {});
		return b
	};
	(function(a) {
	...
	b.createDOMDrawable = function(a, c) {
		...
		j.style.width = h + "px";
		j.style.height = i + "px";
		if (d == "canvas") j.width = h, j.height = i, e && (d = j.getContext("2d"), h = c.rect ||
			[0, 0, h, i], d.drawImage(e, h[0], h[1], h[2], h[3], a.x || 0, a.y || 0, a.width || h[2], a.height || h[3]));
		else if (j.style.opacity = a.alpha != void 0 ? a.alpha : 1, j.style.overflow = "hidden", e && e.src) j.style.backgroundImage = "url(" + e.src + ")", j.style.backgroundPosition = -(a.rectX || 0) + "px " + -(a.rectY || 0) + "px";
		return j
	};
	b.toString = function() {
		return "Quark"
...
(function() {
	...
	c.prototype.union = function(b, a) {
		...
		else this.x = f, this.y = g, this.width = d, this.height = e
	};
...
(function() {
	...
	c.prototype.registerStage = function(b, a, f, c) {
		this.register(b.context.canvas, a, function(a) {
			b.onEvent(a)
		}, f, c)
	};
	c.prototype.register = function(b, a, f, c, d) {
		for (var e = {
			...
			h._onEvent(a, e, f)
		}, d = 0; d < a.length; d++) b.addEventListener(a[d], c, !1)
	};
	c.prototype._onEvent = function(b, a, f) {
		...
		a.stop &&
			(b.stopPropagation(), b.stopImmediatePropagation && b.stopImmediatePropagation())
	};
...
(function() {
	...
	c.prototype.removeEventListener = function(b, a) {
		...
		for (var c = 0; c < f.length; c++)
			if (f[c] === a) return f.splice(c, 1), f.length == 0 && delete this._eventMap[b], !0;
		return !1
	};
...
(function() {
		...
		for (var f = 0; f < a.children.length; f++) {
			...
			if (h.children) c(h, b);
				...
				if (h._rotatedPoints != null) {
					...
					b.moveTo(j.x - 0.5, j.y - 0.5);
					for (var k = 1; k < h._rotatedPoints.length; k++) {
						var l = h._rotatedPoints[k];
						b.lineTo(l.x - 0.5, l.y - 0.5)
					}
					b.lineTo(j.x - 0.5, j.y - 0.5);
					b.stroke();
					b.closePath();
					b.globalAlpha = 0.5
				}
				b.beginPath();
				b.rect((i.x >> 0) - 0.5, (i.y >> 0) - 0.5, i.width >> 0, i.height >> 0);
				b.stroke();
				b.closePath()
			} else if (h.drawable.domDrawable) h.drawable.domDrawable.style.border =
				"1px solid #f00"
		}
	}
	Quark.getUrlParams = function() {
		...
		var a = {},
			...
			f = b.indexOf("?");
		if (f > 0)
			for (var b = b.substring(f + 1).split("&"), f = 0, c; c = b[f]; f++) d = b[f] = c.split("="), c = d, a[c[0]] = c.length > 1 ? c[1] : !0;
		return a
	};
	var b = document.head,
	...
	Quark.toggleDebugRect = function(a) {
		...
		if (a.paused) {
			...
			a._update(b);
			b = b.context;
			if (b != null) b.save(), b.lineWidth = 1, b.strokeStyle = "#f00", b.globalAlpha = 0.5;
			c(a, b);
			b != null && b.restore()
		}
	}
...
(function() {
	...
	c.prototype._run = function() {
		if (!this.paused) {
			...
			for (var f = 0, c = this._listeners.length, d, e; f < c; f++) d = this._listeners[f], e = d.__runTime || 0, e == 0 ? d.step(this.info) : a > e && (d.step(this.info), this._listeners.splice(f, 1), f--, c--);
			b.lastTime = a
		}
	};
...
(function() {
	...
	c.prototype._addSource = function(b) {
		if (b) {
			for (var b = b instanceof Array ? b : [b], a = 0; a < b.length; a++) this._totalSize += b[a].size || 0;
			this._source = this._source ?
				this._source.concat(b) : b
		}
	};
...
(function() {
	var c = Quark.Tween = function(a, f, c) {
		...
		for (var d in f) {
			var e = a[d],
				h = f[d];
			e !== void 0 && typeof e == "number" && typeof h == "number" && (this._oldProps[d] = e, this._newProps[d] = h, this._deltaProps[d] =
				h - e)
		}
	...
	c.prototype.setProps = function(a, b) {
		...
		for (c in b) this._newProps[c] = b[c], this._deltaProps[c] = b[c] - this.target[c]
	};
	...
	c.prototype._update = function() {
		if (!this.paused) {
			...
			if (!(b < 0)) {
				...
				if (a >= 1) {
					if (this.reverse) {
						if (g = this._oldProps, this._oldProps = this._newProps, this._newProps =
							g, this._startTime = Date.now(), this._frameCount = 0, this._reverseFlag *= -1, !this.loop) this.reverse = !1
					} else if (this.loop) {
						for (g in this._oldProps) this.target[g] = this._oldProps[g];
						this._startTime = Date.now();
						this._frameCount = 0
					} else if (c.remove(this), g = this.next, g != null && (g instanceof c ? (a = g, g = null) : a = g.shift(), a != null)) a.next = g, a.start();
	...
	b.Elastic.EaseIn = function(a) {
		...
		!c ||
			c < 1 ? (c = 1, b = d / 4) : b = d / (2 * Math.PI) * Math.asin(1 / c);
	...
	b.Elastic.EaseInOut = function(a) {
		...
		!c || c < 1 ? (c = 1, b = d / 4) : b = d / (2 * Math.PI) * Math.asin(1 / c);
		return (a *= 2) < 1 ? -0.5 * c * Math.pow(2,
			10 * (a -= 1)) * Math.sin((a - b) * 2 * Math.PI / d) : c * Math.pow(2, -10 * (a -= 1)) * Math.sin((a - b) * 2 * Math.PI / d) * 0.5 + 1
	};
	...
	c.prototype._evtHandler = function(b) {
		if (b.type == "progress") {
			var a = 0,
				...
				g = b.target.buffered;
			if (g && g.length > 0)
				for (a = g.length; a >= 0; a--) c = g.end(a) - g.start(a);
			if (c / b.target.duration >= 1) this._element.removeEventListener("progress", this._evtHandler), this._element.removeEventListener("error", this._evtHandler), this._loaded = !0, this.dispatchEvent({
				type: "loaded",
				target: this
...
(function() {
	...
	a.prototype.hitTestPoint = function(a, b,
		c) {
...
(function() {
	var c = Quark.DisplayObjectContainer = function(b) {
		...
		if (b.children)
			for (var a = 0; a < b.children.length; a++) this.addChild(b.children[a])
	};
	...
	c.prototype.addChild = function(b) {
		for (var a = this.children.length, c = 0; c < arguments.length; c++) b = arguments[c], this.addChildAt(b,
			a + c);
		return this
	};
	c.prototype.removeChildAt = function(b) {
		...
		if (a != null) this.getStage().context.remove(a), a.parent = null;
	...
	c.prototype.render =
		function(b) {
			...
			for (var a = 0, f = this.children.length; a < f; a++) this.children[a]._render(b)
	};
	c.prototype.getObjectUnderPoint = function(b, a, c, g, d) {
		if (d) var e = [];
		for (var h = this.children.length - 1; h >= 0; h--) {
			var i = this.children[h];
			if (!(i == null || !i.eventEnabled || !i.visible || i.alpha <= 0))
				if (i.children != void 0 && i.eventChildren && i.getNumChildren() > 0) {
					var j = i.getObjectUnderPoint(b, a, c, g, d);
					if (j)
						if (d) j.length > 0 && (e = e.concat(j));
						else return j;
					else if (i.hitTestPoint(b, a, c, g))
						if (d) e.push(i);
						else return i
				} else if (i.hitTestPoint(b, a, c, g))
				if (d) e.push(i);
...
(function() {
	...
	c.prototype.addFrame = function(b) {
		...
		if (b instanceof Array)
			for (var c = 0; c < b.length; c++) this.setFrame(b[c], a + c);
		else this.setFrame(b, a);
		return this
	};
	c.prototype.setFrame =
		function(b, a) {
	...
	c.prototype.getFrameIndex = function(b) {
		...
		for (var b = this._frameLabels[b], a = this._frames, c = 0; c < a.length; c++)
			if (b == a[c]) return c;
		return -1
	};
	c.prototype.nextFrame = function(b) {
		var a = this._frames[this.currentFrame];
		if (a.interval > 0) b = this._displayedCount + b, this._displayedCount = a.interval > b ? b : 0;
		if (a.jump >= 0 || typeof a.jump == "string")
			if (this._displayedCount == 0 || !a.interval) return this.currentFrame =
				this.getFrameIndex(a.jump);
...
(function() {
	var c = Quark.Context = function(b) {
		if (b.canvas == null) throw "Quark.Context Error: canvas is required.";
		this.canvas = null;
		Quark.merge(this, b)
	};
...
(function() {
	var c = Quark.CanvasContext = function(a) {
	...
	c.prototype.transform = function(a) {
		var c = this.context;
		(a.x != 0 || a.y != 0) && c.translate(a.x, a.y);
		a.rotation %
			360 != 0 && c.rotate(a.rotation % 360 * b);
		(a.scaleX != 1 || a.scaleY != 1) && c.scale(a.scaleX, a.scaleY);
		(a.regX != 0 || a.regY != 0) && c.translate(-a.regX, -a.regY);
...
(function() {
	...
	var e = Quark.DOMContext = function(a) {
	...
	e.prototype.transform = function(a) {
		...
		if (a.transformEnabled || !a._addedToDOM) {
			var d = Quark.cssPrefix,
				e = d + "TransformOrigin";
			d += "Transform";
			if (a.propChanged("visible", "alpha")) b.style.display = !a.visible || a.alpha <= 0 ? "none" : "";
			if (a.propChanged("alpha")) b.style.opacity = a.alpha;
			if (a.propChanged("rectX", "rectY")) b.style.backgroundPosition = -a.rectX + "px " + -a.rectY + "px";
			if (a.propChanged("width", "height")) b.style.width = a.width + "px", b.style.height = a.height + "px";
