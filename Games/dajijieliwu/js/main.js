...
{	
	if(this.begin == null)
	{
		...
		playBtn.setUpState({rect:[0,0,640,59]});//0,135,195,70
		playBtn.setOverState({rect:[0,0,640,59]});
        playBtn.scaleX = sX*1;
        playBtn.scaleY = sY*1;
		playBtn.regX = playBtn.width >> 1;
		playBtn.regY = playBtn.height >> 1;
		playBtn.x = this.width * 0.5;
		playBtn.y = this.height * 0.45;
		...
		//帮助提示
         ...
         tip.setUpState({rect:[0,-59,640,59]});
        tip.scaleX = sX*1;
        tip.scaleY = sY*1;
        tip.x = sX;
        tip.y = 480*sY;
...
game.createPeachs = function()
{
	...
	for(var i = 0; i < this.maxPeachs; i++)
	{
		...
		peach.scaleX = peach.scaleY = this.stage.width*1/this.island.width;
		peach.y=200;
...
game.updatePeachs = function()
{
	...
	for(var i = 0; i < peachs.length; i++)
	{
		...
		if(peach.currentSpeedY > 0) peach.currentSpeedY += 0;

		else if(peach.currentSpeedY < 0) peach.currentSpeedY += 0;

		if(peach.type.score =="-5"){
			 peach.currentSpeedY += 0.1
			};
...
/*	
 var getBtn = new Q.Button({id:"gpBtns", image:this.getImage("out")}); //没超过100分得分享好友
	...
	var shareWindow = document.getElementById('share');
  	...
  	ash_tan.setUpState({rect:[318,363,322,59]});
    ash_tan.scaleX = 1*sX;
    ash_tan.scaleY = 1*sY;
    ash_tan.x = 160*sX;
    ash_tan.y = 710*sY;
    ash_tan.onEvent = function(e)
    {
        Q.Button.prototype.onEvent.call(this, e);
        if(e.type == "mouseup" || e.type == "touchend")
        {
    ...
    var ash_tan2 = new Q.Button({id:"ash_tan2", image:this.getImage("out")}); //游戏结束弹出的文本框
  	ash_tan2.setUpState({rect:[401,317,239,46]});
    ash_tan2.scaleX = 1*sX;
    ash_tan2.scaleY = 1*sY;
    ash_tan2.x = 210*sX;
    ash_tan2.y = 400*sY;
   ...
   lingjiangBtn.onEvent = function(e)
	...
	rePlayBtn.setUpState({rect:[318,422,322,59]});
    rePlayBtn.x = 160*sX;
    rePlayBtn.y = 640 *sY;
 ...
 var rePlayBtns = new Q.Button({id:"rpBtna", image:this.getImage("out")}); //没超过100分再来一次
	...
	rePlayBtns.scaleX = sX*1;
    rePlayBtns.scaleY = sY*1;
	rePlayBtns.setUpState({rect:[318,422,322,59]});
    rePlayBtns.x = 160*sX;
    rePlayBtns.y = 640 *sY;
 ...
 var phBtns = new Q.Button({id:"phBtn", image:this.getImage("out")}); //没超过100分再来一次
	
	phBtns.scaleX = sX*1;
    phBtns.scaleY = sY*1;
	phBtns.setUpState({rect:[0,0,322,59]});
    phBtns.x = 160*sX;
    phBtns.y = 710 *sY;
    phBtns.onEvent = function(e)
    {
        Q.Button.prototype.onEvent.call(this, e);
        if(e.type == "mouseup" || e.type == "touchend")
        {
            window.location.href = '../paihang.php';
        }else if(e.type == "mouseout")
        {
            game.context.canvas.style.cursor = "";
        }
 ...
 if(end_num>=1000000000000000000){
		
		 this.stage.addChild(this.end, /*this.getPriceBtn,*/ this.rePlayBtn, this.endNum, this.lingjiangBtn);
		 
	}
	else{
		
		 this.stage.addChild(this.ends, /*this.getBtn,*/this.ash_tan2, this.rePlayBtns, this.phBtns, this.endNum);
