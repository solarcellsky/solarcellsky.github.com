...
function TimeCount(c) {
    if (parseInt(c) >= 0) {
        var hour = parseInt(c / 21600);
        var minutes = parseInt(c / 3600);
        var seconds = parseInt(c / 60);
        var millisecond = parseInt(c % 60);
        if (seconds >= 60) {
            seconds = seconds % 60
        }
        if (minutes < 10) {
        ...
        var result = seconds + ":" + millisecond;
        return result
    }
}
