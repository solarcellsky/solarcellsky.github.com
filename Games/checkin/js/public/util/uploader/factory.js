...
Uploader.prototype.submit = function(formData) {
  var self = this;
  if (window.FormData && self._files) {
    // build a FormData
    var form = formData?formData:new FormData(self.form.get(0));
...
MultipleUploader.prototype.submit = function(formData) {
  $.each(this._uploaders, function(i, item) {
    item.submit(formData);
  });
  return this;
};
