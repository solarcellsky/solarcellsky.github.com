...
define(function(require, exports) {
     ...
     $.dateRangePickerLanguages =
  {
    ...
    'cn':  //simplified chinese
    {
      ...
      'default-default': '自定义时间段',  //请选择一个日期范围
      'time':'时间',
      'hour':'小时',
      'minute':'分钟'
    },
  ...
  $.fn.dateRangePicker = function(opt)
  {
    ...
    function init_datepicker()
    {
      ...
      box.find('.footer .cancel').click(function(){
        closeDatePicker();
      });
    ...
    function calcPosition()
    {
      if (!opt.inline){
        ...
        if ($(opt.container).css("position") == "relative"){
          ...
          if(!hasMonth2()){
            box.css({
                top: offset.top+$(self).outerHeight() + parseInt($('body').css('border-top')+10 || 10,10 )+10,
                left: $(self).outerWidth()/2 + $(self).offset().left - 222 +48
            });
          }else{
            box.css({
                top: offset.top+$(self).outerHeight() + parseInt($('body').css('border-top')+10 || 10,10 )+10,
                left: $(self).outerWidth()/2 + $(self).offset().left - 447 +48
            });
          }
    ...
    function isValidTime(time)
    {
      ...
      if (opt.start && !opt.end && !opt.singleDate)
      {
        ...
        if (opt.selectBackward && time > opt.start) return false;   //选定向后选择日期，时间>选择的时间,无效选中
    ...
    function createDom()
    {
      ...
      if (opt.showTopbar)
      {
        html +='<div class="arrow"><em></em><span></span></div>'
      ...
      html +=    '<a href="javascript:void(0);" class="confirm apply-btn disabled ">确定</a>'
      html +=    '<a href="javascript:void(0);" class="cancel">取消</a>'
      html += '</div>';
      html += '</div>';
      //html += '<div class="footer"><a href="#" class="confirm button">确定</a><a href="#" class="cancel button">取消</a>';
    ...
    function myisValidTime(time){
      //time = parseInt(time, 10);
      var d = new Date();
      var today = d.getTime(); //返回从 1970 年 1 月 1 日至今的毫秒数
      // if (compare_day(time, today) > 0) {
      //   return false;
      // };
      // return true;
      var p = parseInt(time)/86400000 - parseInt(today)/86400000;
      if(p > 0)
        return false;
      if(p < -90)     //设置可选最长时间
        return false;
    ...
    function createMonthHTML(d)
    {
      ...
      var now = new Date();
      var dayOfWeek = d.getDay();   //返回整数0~6,表示周日到周一
      ...
      var toMonth = d.getMonth();  //返回整数0~11，表示一月到十二月
      for(var i=0; i<40; i++)
      {
