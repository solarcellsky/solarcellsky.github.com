...
function BrokeredMessage(body, properties) {
    ...
    /// The message payload. Any object used as a body of the message.
    /// </param>
...
function QueueClient(config) {
    ...
    function MessagingResult(result, httpStatusCode, msg, response) {
        this.result = result;
    ...
    var getUri = function (entityName, head) {
        var entityUri = "https://" + m_ServiceNamespace + ".servicebus.chinacloudapi.cn/" + entityName;
        var uri;
        if (head == null)
    ...
    var generateToken = function (entityPath) {
        ...
        var uri = "http://" + m_ServiceNamespace + ".servicebus.chinacloudapi.cn/" + entityPath;

        var endocedResourceUri = encodeURIComponent(uri);

        var expireInSeconds = (Date.now() / 1000) + 3600;
        ...
        if (props != null) {
            for (var i = 0; i < props.length; i++) {
                if (props[i].Name == null || props[i].Value == null) {
                    ...
                    if (callback != null) {
                        var messagingResult = new MessagingResult("The list of properties must contain pair values {'Name': ***, 'Value': ***}", this.status, null, this.response);
