...
var SHARE_DESK = "愤怒的小鸟击退绿猪，立享1元吃抹茶甜筒！";
...
$(function()
{
    ...
    if(isZhifubao())
    {
        document.addEventListener('AlipayJSBridgeReady', function () {AlipayJSBridge.call("hideOptionMenu")});
    }
...
function musicFunc() {
    ...
    $("#music_con").on("touchstart", function(e) { e.stopPropagation();})
...
function quaternionSub(q1, q2)
{
    ...
    if(subw < 50 && subz < 1500 && suby < 1500 && subx < 1500) {return true;} else {return false;}
}
...
function quaternionSubBig(q1, q2)
{
    ...
    var subz = Math.abs(q4.z * 10000);
    var subw = Math.abs(10000 - (q4.w * 10000));
    if(subw < 100 && subz < 2000 && suby < 2000 && subx < 2000) {return true;} else {return false;}
}
...
function shotEnd()
{
    shotani.destory(); $('#tape').show(); $('#tape2').hide();
}
...
function onTouchEnd(event)
{
    ...
    //皮筋动画
    if(forceani) {$(forceani.con).unbind("webkitAnimationEnd", forceEnd);forceani.destory();$('#tape').hide();}
    if(shotani) {$(shotani.con).unbind("webkitAnimationEnd", shotEnd);}
    $('#tape').hide();$('#tape2').hide();
    ...
    setTimeout(function() {if(hitpig) {scene.remove(hitpig);}}, 700);
...
function gameOverAnimate()
{
    ...
    switch (prizenum.length)
    {
        ...
        case 1:
            $('#overcon').show();
            $('#bbdd').hide();
            TweenMax.to($('#oframe'), 0, {scale:.8, alpha:0, onComplete:function()
            {
                TweenMax.to($('#oframe'), .1, {scale:1.5, alpha:1, onComplete:function()
                {
                    TweenMax.to($('#oframe'),.3, {scale:1.01, alpha:1});
                }});
            }});
            TweenMax.to($('#olight'), 0, {left:-500, alpha:0, onComplete:function()
            {
                TweenMax.to($('#olight'),.5, {scale:1.01, alpha:1, left:-30});
            }});

            $('#obird1').show();
            TweenMax.to($('#obird1'), 0, {top:120, alpha:0, dealy:1, onComplete:function()
            {
                TweenMax.to($('#obird1'), .2, {alpha:1, top:30, ease:Circ.easeOut, onComplete:function()
                {
                    TweenMax.to($('#obird1'), .5, {top:70, ease:Circ.easeOut});
                }});
            }});
            $('#contxt1').show();
            TweenMax.to($('#contxt1'), 0, {top:280, scaleY:0, alpha:0, dealy:1, onComplete:function()
            {
            ...
            if(prizenum[0] == 1)
            {
                $('#oprize1').show().css({top:550, opacity:0, '-webkit-transform':'rotate(-7deg)'}).animate({top:480, opacity:1});
            }
            else
            {
                $('#oprize2').show().css({top:550, opacity:0, '-webkit-transform':'rotate(-7deg)'}).animate({top:480, opacity:1});
            }
        ...
        case 2:
            ...
            $('#obird2').show();
            TweenMax.to($('#obird2'), 0, {top:120, alpha:0, dealy:1, onComplete:function()
            {
                TweenMax.to($('#obird2'), .2, {alpha:1, top:30, ease:Circ.easeOut, onComplete:function()
                {
                    TweenMax.to($('#obird2'), .5, {top:70, ease:Circ.easeOut});
                }});
            }});
            $('#contxt2').show();
            TweenMax.to($('#contxt2'), 0, {top:280, scaleY:0, alpha:0, dealy:1, onComplete:function()
            {
            ...
            for(var i=0; i<prizenum.length; i++)
            {
                ...
                $('#overcon').append(img);
                $(img).css({'position':'absolute', top:550, left:160 + i*110, opacity:0}).delay(300 + i*30).animate({top:480, opacity:1});
                switch (i)
                {
                    case 0:
        ...
        case 3:
            ...
            $('#obird3').show();
            TweenMax.to($('#obird3'), 0, {top:120, alpha:0, dealy:1, onComplete:function()
            {
                TweenMax.to($('#obird3'), .2, {alpha:1, top:10, ease:Circ.easeOut, onComplete:function()
                {
            ...
            $('#contxt3').show();
            TweenMax.to($('#contxt3'), 0, {top:280, scaleY:0, alpha:0, dealy:1, onComplete:function()
            {
            ...
            for(var i=0; i<prizenum.length; i++)
            {
                ...
                (prizenum[i] == 1)?img.src = 'img/attack_tips/ticket.png':img.src = 'img/attack_tips/ticket2.png';
                $('#overcon').append(img);
                $(img).css({'position':'absolute', top:550, left:100 + i*110, opacity:0}).delay(300 + i*30).animate({top:480, opacity:1});
                switch (i)
                {
                    case 0:
                        $(img).css({'-webkit-transform':'rotate(-20deg)'})
    ...
    $('.sharebtn').bind('touchend', function()
    {
        ...
        if(isZhifubao()) {onShareAlipay();return;}

        $('#sharecon').show();
        TweenMax.to($('#sbrid'), 0, {top:680, left:30, scale:0.8, alpha:.9, onComplete:function()
        {
            TweenMax.to($('#sbrid'), 1.5, {
                bezier:[{top:680, left:30, scale:0.8, alpha:.9}, {top:150, left:200, scale:0.5, alpha:.8}, {top:50, left:530, scale:0, alpha:0}],
                ease:Strong.easeOut});
        }})
        ...
        {
            $('#shareboom').show();
            TweenMax.to($('#shareboom'),0, {scale:0,onComplete:function()
            {
                TweenMax.to($('#shareboom'),.3, {scale:1.5, onComplete:function()
                {
    ...
    $('#sharecon').bind('touchend', function(){$('#sharecon').hide();})
...
{
    var loarr = [{top:-50, left:680}, {top:600, left: 900}, {top:1080, left: 700}, {top:1300, left: 300}, {top:700, left: -300}, {top:-330, left: -500}, {top:1400, left: -150}]
